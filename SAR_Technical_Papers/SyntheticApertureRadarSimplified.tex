...
The size of the Synthetic Aperture Radar (SAR)  image matrix in terms of the number of range cells and azimuth cells is derived from parameters such as frequency,  chirp bandwidth, range, and  antenna sizes. The equations for the number of range and azimuth cells clearly show the tradeoffs in the design of SAR systems as well as predict the complexity of image formation. The SEASAT SAR system is illustrated.
...
The geometry of the SEASAT SAR is illustrated in Figure \ref{fig:sar_geometry_range}. In Figure \ref{fig:geometry_range_plane} the cross section along the range direction is shown (the range plane). The "look angle" is $\theta$ and $\psi$ is the beam width along the range direction. The beam width is,
\begin{equation}
\label{ }
\psi=a_y\frac{\lambda}{D_y}
\end{equation}
...
\begin{equation}
\label{ }
R_0\psi = L_y \cos \theta
\end{equation}

from which 
\begin{equation}
\label{ }
L_y=\frac{R_0\psi}{\cos \theta }
\end{equation}
...
\begin{equation}
\label{ }
\delta r = \frac{c}{2} \delta T = \frac{c}{2B}
\end{equation}
...
The SEASAT SAR geometry showing the azimuth beam width is shown in Figure \ref{fig:sar_geometry_azimuth}. The viewing geometry in the azimuth plane  is shown in Figure   \ref{fig:geometry_azimuth}. From the figure we have the length of the illuminated area along the azimuth direction at range $R_0$ is,
...
where $\varphi$ is the beam width,
\begin{equation}
\label{eq:phi}
\varphi= a_x \frac{\lambda}{D_x}
\end{equation}
...
\begin{equation}
\label{ }
f_D=\frac{v\sin \frac{\varphi}{2}}{\lambda}
\end{equation}

The doppler bandwidth $B_D$ is twice $f_D$. Thus, assuming that $\varphi$ is small,
\begin{equation}
\label{ }
B_D = \frac{v\varphi}{\lambda}
\end{equation}

Or substituting for $\varphi$,
\begin{equation}
\label{ }
...
\chapter{Azimuth Resolution}
From the number of azimuth cells $N_x$ and   the length of the illuminated area along the azimuth direction, $L_x$, we can obtain the azimuth  resolution $\delta_{az}$:

\begin{equation}
\label{ }
\delta_{az}=\frac{L_x}{N_x}
\end{equation}

Now from Equation \ref{eq:Lx},  substituting for $\varphi$ from Equation \ref{eq:phi}  we have,

\begin{equation}
\label{ }
L_x=R_0 a_x \frac{\lambda}{D_x}
\end{equation}

Substituting for $L_x$ and $N_x$ we have,


\begin{equation}
\label{ }
\delta_{az}=\frac{R_0 a_x \frac{\lambda}{D_x}}{2a_x^2 \frac{R_0\lambda}{D_x^2}}
\end{equation}
...
\begin{equation}
\label{ }
\delta_{az}=\frac{D_x}{2a_x} 
\end{equation}

If we assume $a_x \approx 1 $, then 

\begin{equation}
\label{ }
\delta_{az}=\frac{D_x}{2} 
\end{equation}

This  result   shows that the azimuth resolution only depends on the physical size of the antenna in the along-track direction. 
 ...
 \begin{figure}[hbtp]
  \centering 
    \includegraphics[width=4in]{./sar_geometry_range.jpg}
  \caption{SAR Geometry  Derived From NASA/JPL}
\label{fig:sar_geometry_range}
\end{figure}
...
\end{figure}
 ...
 \begin{figure}[hbtp]
  \centering 
    \includegraphics[width=4in]{./sar_geometry_azimuth.jpg}
  \caption{SAR Geometry Azimuth}
\label{fig:sar_geometry_azimuth}
\end{figure}

 \begin{figure}[hbtp]
  \centering 
    \includegraphics[width=4in]{./azimuth_resolution.jpg}
